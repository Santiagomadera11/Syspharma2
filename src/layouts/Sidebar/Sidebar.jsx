...
import { 
  ...
  Truck, DollarSign, ClipboardList, Stethoscope, Calendar, 
...
const Sidebar = () => {
  ...
  const [openMenus, setOpenMenus] = useState({ compras: true, ventas: false, servicios: false });
  ...
  return (
    <aside className="w-60 h-screen bg-[#2C3E50] flex flex-col text-white shadow-xl flex-shrink-0 border-l border-gray-700">
      ...
      <div className="p-3 border-t border-gray-700 bg-[#243342]">
        <button className="flex items-center justify-center gap-2 w-full py-2 text-xs font-medium text-red-400 hover:bg-red-500/10 rounded-lg transition-colors">
          <LogOut size={16} /> 
          Cerrar Sesión
        </button>
      </div>
...
const MenuItem = ({ to, icon: Icon, label, active }) => (
  <Link to={to} className={`flex items-center px-3 py-2 rounded-md transition-all duration-200 group ${active ? 'bg-primary-500 text-white shadow-sm' : 'text-gray-300 hover:bg-white/5 hover:text-white'}`}>
...
const SubMenuItem = ({ to, label, icon: Icon, active }) => (
  <Link to={to} className={`flex items-center pl-10 pr-3 py-1.5 text-xs transition-colors rounded-md mb-0.5 ${active ? 'text-primary-400 font-bold bg-white/5' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
...
const MenuGroup = ({ to, title, icon: Icon, isOpen, onToggle, children, active }) => (
  <div className="mb-0.5">
    ...
    <div className={`flex items-center rounded-md transition-colors ${active ? 'bg-primary-500 text-white' : 'text-gray-300 hover:bg-white/5 hover:text-white'}`}>
