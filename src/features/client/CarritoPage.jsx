...
import { LS, read, write, pushNotification } from '../../shared/services/lsService';
...
const CarritoPage = () => {
  ...
  function load() {
    ...
    const normalized = (cart || []).map((item) => {
      if (item && (typeof item === 'string' || typeof item === 'number')) {
        const p = prods.find((x) => x.id === item || x.id === Number(item)) || {};
      ...
      const currentPrice = Number(p.precio ?? p.price ?? precioStored ?? 0) || 0;
      ...
      const priceChanged = precioStored !== undefined && p && (Number(precioStored) !== Number(p.precio ?? p.price));
      return { ...item, producto: p, precioActual: currentPrice, priceChanged, cantidad: adjustedQty };
    });
    ...
    try {
      ...
      const rawNorm = (raw || []).map((it) => (it && typeof it === 'object' ? it : { id: it, cantidad: 1 }));
      let changed = false;
  ...
  const changeQty = (id, delta) => {
    ...
    let arr = raw.map((it) => {
      if (it && (typeof it === 'string' || typeof it === 'number')) {
        ...
        return { id: p.id ?? it, cantidad: 1, precio: p.precio ?? p.price ?? 0 };
      }
      return it;
    });

    arr = arr.map((it) => (it.id === id ? { ...it, cantidad: Math.max(1, (it.cantidad || 1) + delta) } : it));
    write(LS.CART, arr);
  };
  ...
  const handleRemove = (id) => {
    ...
    const arr = raw.filter((f) => {
      if (f && (typeof f === 'string' || typeof f === 'number')) return f !== id;
            ...
            <img src={(it.producto && it.producto.imagen) || it.imagen} className="w-20 h-20 object-cover" alt="" />
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-emerald-600 font-bold text-lg">${Number(it.precioActual || 0).toFixed(2)}</div>
                      </div>
                      <div className="mt-2">
                        {(() => {
            ...
            <div className="flex flex-col items-end">
              <div className="flex items-center gap-2">
                ...
                {(() => {
                  ...
                  return (
                    <button onClick={() => changeQty(it.id, 1)} disabled={disabled} className={`px-2 py-1 border rounded ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
                      +
                    </button>
                  );
                })()}
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              ...
              setSelectedPayment(methods && methods.length ? methods[0].value : null);
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg w-full max-w-lg p-6">
                ...
                {cartItems.map((it) => (
                  <div key={it.id} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <img src={(it.producto && it.producto.imagen) || it.imagen} alt="" className="w-12 h-12 object-cover rounded" />
                      <div>
                        <div className="font-semibold">{(it.producto && it.producto.nombre) || it.nombre}</div>
                        <div className="text-xs text-gray-500">{it.cantidad} x {formatCurrency(it.precioActual || 0)}</div>
                      </div>
                    </div>
                    <div className="font-bold">{formatCurrency((it.precioActual || 0) * (it.cantidad || 1))}</div>
                  </div>
                ))}
              </div>
            ...
            <div className="mb-4">
              ...
              <select value={selectedPayment || ''} onChange={(e) => setSelectedPayment(e.target.value)} className="w-full border px-3 py-2 rounded">
            ...
            {checkoutError && <div className="text-sm text-red-600 mb-2">{checkoutError}</div>}

            <div className="flex justify-end gap-2">
              <button onClick={() => setCheckoutOpen(false)} className="px-4 py-2 border rounded">Cancelar</button>
              <button onClick={async () => {
                ...
                try {
                  ...
                  const normCart = (latestCart || []).map(it => (it && typeof it === 'object') ? it : { id: it, cantidad: 1 });

                  // Validate stock
                  for (const item of normCart) {
                    const prod = latestProducts.find(p => p.id === item.id);
                    const available = prod ? (prod.stock ?? prod.existencia ?? 0) : 0;
                    if ((item.cantidad || 0) > available) {
                      setCheckoutError(`Stock insuficiente para ${prod ? (prod.nombre||prod.name) : item.id}`);
                      setProcessing(false);
                      return;
                    }
                  }
                  ...
                  const updatedProducts = latestProducts.map(p => {
                    ...
                    if (cartItem) {
                      const newStock = Math.max(0, (p.stock ?? p.existencia ?? 0) - (cartItem.cantidad || 0));
                  ...
                  const computedTotal = normCart.reduce((s, it) => {
                    const prod = latestProducts.find(p => p.id === it.id) || {};
                    const price = Number(it.precio ?? it.precioActual ?? prod.precio ?? prod.price ?? 0) || 0;
                  ...
                  const order = {
                    ...
                    productos: normCart.map(it => {
                      const prod = latestProducts.find(p => p.id === it.id) || {};
                      const price = Number(it.precio ?? it.precioActual ?? prod.precio ?? prod.price ?? 0) || 0;
                      return { id: it.id, nombre: prod.nombre || prod.name || null, cantidad: it.cantidad || 1, precio: price };
                    }),
                    total: computedTotal,
                  ...
                  // Push notification
                  pushNotification({ title: 'Compra exitosa', message: '¡Compra exitosa! Tu pedido ha sido registrado y está en proceso', date: new Date().toISOString(), path: '/client/mis-pedidos' });

                  setProcessing(false);
                  setCheckoutOpen(false);
                  setToast({ message: 'Compra registrada correctamente', type: 'success', zIndex: 70 });
                } catch (e) {
                  setProcessing(false);
                  setCheckoutError('Error procesando la compra');
                }
              }} disabled={processing || !selectedPayment} className={`px-4 py-2 bg-emerald-600 text-white rounded ${processing || !selectedPayment ? 'opacity-60 cursor-not-allowed' : ''}`}>
