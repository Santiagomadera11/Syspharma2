...
export const ProductsPage = () => {
  ...
  const handleDelete = (id) => {
    if(window.confirm("¿Borrar producto del inventario?")) {
      ...
      setProducts(newList);
      if (newList.length <= currentPage * itemsPerPage && currentPage > 0) setCurrentPage(currentPage - 1);
    ...
    <div className="h-full flex flex-col gap-3 font-sans relative">
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        <div>
          ...
          <p className="text-gray-500 text-xs">Gestión de productos farmacéuticos</p>
        </div>
        <button onClick={handleOpenCreate} className="bg-[#34D399] hover:bg-emerald-500 text-white px-4 py-1.5 rounded-lg font-bold shadow-sm text-xs flex items-center gap-1.5 transition-all">
          <Plus size={16} /> Nuevo
        </button>
      </div>
            ...
            <tbody className="divide-y divide-gray-100 text-xs">
              {displayedProducts.map((prod) => (
                <tr key={prod.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400 font-mono">{prod.codigo}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-2.5 text-gray-600">{prod.laboratorio}</td>
                  <td className="px-4 py-2.5">
                    <span className="px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 border border-blue-100 text-[10px] font-bold">
                      {prod.categoria}
                    </span>
                  </td>
                  <td className="px-4 py-2.5 font-bold text-gray-800">{formatCurrency(prod.precio)}</td>
                  
                  {/* Stock con Colores */}
                  <td className="px-4 py-2.5 text-center">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                      prod.stock <= 5 ? 'bg-red-100 text-red-600' : 
                      prod.stock <= 20 ? 'bg-orange-100 text-orange-600' : 'bg-green-100 text-green-600'
                    }`}>
                      {prod.stock} un.
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-2.5 text-center">
                    <button onClick={() => handleToggleStatus(prod.id)} className={`relative w-8 h-4 rounded-full transition-colors duration-200 focus:outline-none ${prod.estado ? 'bg-gray-700' : 'bg-gray-300'}`}>
                      <span className={`absolute top-0.5 left-0.5 bg-white w-3 h-3 rounded-full shadow transition-transform duration-200 ${prod.estado ? 'translate-x-4' : 'translate-x-0'}`} />
                    </button>
                  </td>

                  <td className="px-4 py-2.5 text-right">
                    <div className="flex items-center justify-end gap-1.5">
                      <button onClick={() => handleOpenEdit(prod)} className="bg-emerald-50 hover:bg-emerald-100 text-emerald-600 p-1.5 rounded-md border border-emerald-200"><Edit size={14} /></button>
                      <button onClick={() => handleDelete(prod.id)} className="bg-red-50 hover:bg-red-100 text-red-600 p-1.5 rounded-md border border-red-200"><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <ProductFormModal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} onSave={handleSave} productToEdit={editingProduct} />
    </div>
  );
};