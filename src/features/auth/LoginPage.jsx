...
export const LoginPage = () => {
  ...
  const handleLogin = (e) => {
    ...
    navigate('/admin/dashboard'); 
        // Simulación de usuario (Más adelante vendrá de la Base de Datos real)
    const userMock = {
      nombre: "Carlos Administrador",
      email: credentials.email,
      rol: "admin", // Puede ser 'admin', 'empleado', 'cliente'
      foto: null
    };

    // Guardamos al usuario en la memoria
    localStorage.setItem('syspharma_user', JSON.stringify(userMock));
    ...
    <div className="min-h-screen flex w-full overflow-hidden font-sans">
      ...
      <div className="w-full lg:w-[30%] flex items-center justify-center bg-white px-6 md:px-10 shadow-2xl z-20 relative">
        ...
        <div className="w-full">
          <div className="text-center mb-8 mt-8"> {/* mt-8 para dar espacio al botón de volver */}
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div>
              ...
              <div className="relative group">
                ...
                <input type="email" name="email" required placeholder="admin@syspharma.com" className="w-full pl-9 pr-3 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-100 focus:border-primary-400 outline-none transition-all text-sm bg-gray-50 focus:bg-white" onChange={handleChange} />
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-1 ml-1">
                ...
                <a href="#" className="text-[10px] text-primary-600 hover:text-primary-800 font-bold hover:underline">¿Olvidaste tu contraseña?</a>
              </div>
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <Lock size={16} className="text-gray-400 group-focus-within:text-primary-500 transition-colors" />
                </div>
                <input type="password" name="password" required placeholder="••••••••" className="w-full pl-9 pr-3 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-100 focus:border-primary-400 outline-none transition-all text-sm bg-gray-50 focus:bg-white" onChange={handleChange} />
              </div>
            </div>

            <button type="submit" className="w-full bg-primary-600 hover:bg-primary-700 text-white font-bold py-3 rounded-lg transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 mt-4 text-sm">
              Iniciar Sesión <ArrowRight size={16} />
            </button>

            <div className="text-center pt-4 border-t border-gray-100 mt-6">
              <p className="text-xs text-gray-500">
                ¿Eres cliente nuevo? <Link to="/registro" className="text-primary-600 font-bold hover:underline">Regístrate aquí</Link>
              </p>
            </div>
          </form>
        </div>
